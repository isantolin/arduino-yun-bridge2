...
function topic.validate(self, value, section)
	...
	if value:find("[#+]") then
		return nil, translate("Topic prefix cannot contain wildcards (# or +).")
	end
	return value
end
...
function serial.validate(self, value, section)
	if not value or #value == 0 then
		return nil, translate("Serial port cannot be empty.")
	end
	if not value:match("^/dev/tty") then
		return nil, translate("Serial port should start with /dev/tty.")
	end
	return value
end
...
function baud.validate(self, value, section)
	...
	if not allowed[value] then
		return nil, translate("Invalid baudrate. Choose a standard value.")
	end
	return value
end
...
-- MQTT Host: must be a valid hostname or IP
host = s:option(Value, "mqtt_host", "MQTT Host")
host.datatype = "host"
host.placeholder = "127.0.0.1"
host.rmempty = false

-- MQTT Port: must be integer 1-65535
port = s:option(Value, "mqtt_port", "MQTT Port")
port.datatype = "port"
port.placeholder = "1883"
port.rmempty = false
