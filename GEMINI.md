...
22. **Deterministic Virtualenv Resolution in Shell Scripts:** All shell automation and CI scripts (tools/*.sh) MUST implement deterministic virtual environment resolution, prioritizing active $VIRTUAL_ENV, followed by local Tox environments (.tox/coverage/bin/python, .tox/py313/bin/python) before falling back to system binaries, guaranteeing reproducible and isolated execution across both CLI and CI runners.
23. **CI/CD Workflow & Toolchain Single-Source-of-Truth Parity:** No CI/CD workflow file (.github/workflows/*.yml), build automation script (tools/*.sh), or environment configuration may define isolated, hardcoded, or out-of-sync versions for tools (such as protoc), linters, compilers, or runtime dependencies. All dependency versions MUST derive strictly from the canonical manifest (requirements/runtime.toml / pyproject.toml). The dependency synchronization tool (tools/sync_runtime_deps.py) MUST audit and synchronize workflow definitions bidirectionally, guaranteeing that tox -e deps-sync fails if any GitHub Actions step drifts from the canonical version or references pre-release release candidates.
...
29. **Mandatory Post-Execution System Status & Health Invariant:** All hardware validation runs, integration pipelines, and E2E test executions MUST conclude with a mandatory audit of `/tmp/mcubridge_status.json` and active system health via `tools/audit_bridge_status.py`. Passing client test scripts while the persistent daemon records unresolved handshake errors (`last_error != ""`), failure streaks (`failure_streak > 0`), fatal resets (`fatal_count > 0`), link disconnections, dropped spool limits (`cloud_spool_dropped_limit > 0`), or trim events (`cloud_spool_trim_events > 0`) is strictly considered a test failure. No task may be considered complete without a clean, zero-anomaly status snapshot.
