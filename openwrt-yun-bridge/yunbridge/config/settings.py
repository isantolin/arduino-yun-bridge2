...
class RuntimeConfig:
    ...
    def _validate_queue_limits(self) -> None:
        mailbox_limit = self._require_positive("mailbox_queue_limit", self.mailbox_queue_limit)
        mailbox_bytes_limit = self._require_positive("mailbox_queue_bytes_limit", self.mailbox_queue_bytes_limit)
        ...
        console_limit = self._require_positive("console_queue_limit_bytes", self.console_queue_limit_bytes)
        mqtt_limit = self._require_positive("mqtt_queue_limit", self.mqtt_queue_limit)
    ...
    def _normalize_paths(self) -> None:
        root = self._normalize_path(self.file_system_root, field="file_system_root", require_absolute=True)
        spool = self._normalize_path(self.mqtt_spool_dir, field="mqtt_spool_dir", require_absolute=False)
    ...
    def _validate_operational_limits(self) -> None:
        ...
        if self.watchdog_enabled:
            interval = self._require_positive_float("watchdog_interval", float(self.watchdog_interval))
            self.watchdog_interval = interval

        self.bridge_summary_interval = max(0.0, float(self.bridge_summary_interval))
        self.bridge_handshake_interval = max(0.0, float(self.bridge_handshake_interval))
    ...
    @staticmethod
    def _normalize_path(value: str, *, field: str, require_absolute: bool) -> str:
...
def _resolve_watchdog_settings() -> tuple[bool, float]:
    ...
    if disable_flag and disable_flag.strip().lower() in {"1", "true", "yes", "on"}:
    ...
    procd_raw = os.environ.get("PROCD_WATCHDOG") or os.environ.get("YUNBRIDGE_PROCD_WATCHDOG_MS")
    if procd_raw:
        try:
...
def load_runtime_config() -> RuntimeConfig:
    ...
    serial_secret_str = lookup_credential(
        ("YUNBRIDGE_SERIAL_SECRET", "SERIAL_SHARED_SECRET", "serial_shared_secret"),
        credential_map=credentials_map,
        environ=os.environ,
        fallback=raw.get("serial_shared_secret") or "",
    )
    serial_secret_bytes = serial_secret_str.encode("utf-8") if serial_secret_str else b""

    spool_dir = os.environ.get("YUNBRIDGE_MQTT_SPOOL_DIR") or raw.get("mqtt_spool_dir", DEFAULT_MQTT_SPOOL_DIR)
    spool_dir = (spool_dir or DEFAULT_MQTT_SPOOL_DIR).strip()
    if not spool_dir:
        spool_dir = DEFAULT_MQTT_SPOOL_DIR

    mqtt_cafile = lookup_credential(
        ("YUNBRIDGE_MQTT_CAFILE", "MQTT_CAFILE", "mqtt_cafile"),
        credential_map=credentials_map,
        environ=os.environ,
    ...
    mqtt_certfile = lookup_credential(
        ("YUNBRIDGE_MQTT_CERTFILE", "MQTT_CERTFILE", "mqtt_certfile"),
        credential_map=credentials_map,
        environ=os.environ,
        fallback=raw.get("mqtt_certfile"),
    )
    mqtt_keyfile = lookup_credential(
        ("YUNBRIDGE_MQTT_KEYFILE", "MQTT_KEYFILE", "mqtt_keyfile"),
        credential_map=credentials_map,
        environ=os.environ,
    ...
    mqtt_user = lookup_credential(
        ("YUNBRIDGE_MQTT_USER", "MQTT_USERNAME", "mqtt_user"),
        credential_map=credentials_map,
        environ=os.environ,
        fallback=raw.get("mqtt_user"),
    )
    mqtt_pass = lookup_credential(
        ("YUNBRIDGE_MQTT_PASS", "MQTT_PASSWORD", "mqtt_pass"),
        credential_map=credentials_map,
        environ=os.environ,
    ...
    summary_interval = parse_float(raw.get("bridge_summary_interval"), float(DEFAULT_BRIDGE_SUMMARY_INTERVAL))
    ...
    handshake_interval = parse_float(raw.get("bridge_handshake_interval"), float(DEFAULT_BRIDGE_HANDSHAKE_INTERVAL))
    ...
    return RuntimeConfig(
        ...
        file_write_max_bytes=max(1, _get_int("file_write_max_bytes", DEFAULT_FILE_WRITE_MAX_BYTES)),
        file_storage_quota_bytes=max(1, _get_int("file_storage_quota_bytes", DEFAULT_FILE_STORAGE_QUOTA_BYTES)),
        ...
        console_queue_limit_bytes=_get_int("console_queue_limit_bytes", DEFAULT_CONSOLE_QUEUE_LIMIT_BYTES),
        mailbox_queue_limit=_get_int("mailbox_queue_limit", DEFAULT_MAILBOX_QUEUE_LIMIT),
        mailbox_queue_bytes_limit=_get_int("mailbox_queue_bytes_limit", DEFAULT_MAILBOX_QUEUE_BYTES_LIMIT),
        pending_pin_request_limit=_get_int("pending_pin_request_limit", DEFAULT_PENDING_PIN_REQUESTS),
        serial_retry_timeout=parse_float(raw.get("serial_retry_timeout"), DEFAULT_SERIAL_RETRY_TIMEOUT),
        serial_response_timeout=parse_float(raw.get("serial_response_timeout"), DEFAULT_SERIAL_RESPONSE_TIMEOUT),
        serial_retry_attempts=max(1, _get_int("serial_retry_attempts", DEFAULT_SERIAL_RETRY_ATTEMPTS)),
        serial_handshake_min_interval=max(0.0, parse_float(raw.get("serial_handshake_min_interval"), DEFAULT_SERIAL_HANDSHAKE_MIN_INTERVAL)),
        serial_handshake_fatal_failures=max(1, _get_int("serial_handshake_fatal_failures", DEFAULT_SERIAL_HANDSHAKE_FATAL_FAILURES)),
        watchdog_enabled=watchdog_enabled,
        watchdog_interval=watchdog_interval,
        topic_authorization=topic_authorization,
        serial_shared_secret=serial_secret_bytes,
        mqtt_spool_dir=spool_dir,
        process_max_output_bytes=max(1024, _get_int("process_max_output_bytes", DEFAULT_PROCESS_MAX_OUTPUT_BYTES)),
        process_max_concurrent=max(1, _get_int("process_max_concurrent", DEFAULT_PROCESS_MAX_CONCURRENT)),
