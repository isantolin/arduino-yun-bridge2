...
> **Nota:** Todas las dependencias del daemon se instalan vía `opkg`, salvo las bibliotecas que solo existen en PyPI (`aiomqtt==2.5.0`, `paho-mqtt==2.1.0`, `pyserial-asyncio==0.6`, `cobs==1.2.2`, `tenacity==9.1.2`, `prometheus-client>=0.21.0`, `persist-queue==1.1.0`, `attrs==24.2.0`). Esos paquetes se obtienen automáticamente de PyPI durante `3_install.sh`, que ahora lee `../requirements/runtime.toml` y aplica los mismos pines usados por `requirements/runtime.txt`. El resto (`python3-pyserial==3.5`, `python3-psutil>=6.0.0`, `python3-more-itertools>=10.8.0`, etc.) sigue llegando como IPKs tradicionales. El inventario completo vive en `../requirements/runtime.toml`; ejecuta `./tools/sync_runtime_deps.py` tras modificarlo para regenerar `requirements/runtime.txt`, refrescar el `Makefile` e instruir a `1_compile.sh`.
>
> **⚠️ Nota sobre pyserial 3.5:** Esta dependencia no ha recibido actualizaciones desde 2020 y el proyecto no tiene mantenedor activo. Aunque sigue siendo estable para OpenWrt, se recomienda monitorear `pyserial-asyncio` como alternativa futura o preparar un fork interno si surgen incompatibilidades con Python 3.13+. El daemon usa solo la API básica (`serial.Serial`), minimizando riesgos.
