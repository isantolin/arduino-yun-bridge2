...
- **Respawn controlado por procd:** El init script lee `YUNBRIDGE_RESPAWN_THRESHOLD`, `YUNBRIDGE_RESPAWN_WINDOW` y `YUNBRIDGE_RESPAWN_RETRY` antes de llamar a `procd_set_param respawn`, lo que te permite endurecer o relajar la ventana de reintentos sin editar `/etc/init.d/yunbridge` (los valores por defecto siguen siendo `5 30 5`).
...
- **Guía rápida de UCI**:
	...
	export YUNBRIDGE_SERIAL_HANDSHAKE_MIN_INTERVAL='0.0'
	export YUNBRIDGE_SERIAL_HANDSHAKE_FATAL_FAILURES='3'
	...
	- El instalador (`3_install.sh`) inicializa estos valores si aún no existen; personalízalos antes de ejecutar el daemon exportando `YUNBRIDGE_SERIAL_RETRY_TIMEOUT`, `YUNBRIDGE_SERIAL_RETRY_ATTEMPTS`, `YUNBRIDGE_SERIAL_RESPONSE_TIMEOUT`, `YUNBRIDGE_SERIAL_HANDSHAKE_MIN_INTERVAL` o `YUNBRIDGE_SERIAL_HANDSHAKE_FATAL_FAILURES`.
...
| Toolchain OpenWrt/AVR | `1_compile.sh` descarga por defecto el SDK 24.10.5 (`gcc-13.3.0` para `ath79/generic`). | Validar paridad con la rama estable 23.05 (`gcc-13.x`) antes de hacer release, compilando ambos artefactos. | `./1_compile.sh 24.10.5` (por defecto) y `./1_compile.sh 23.05.5` generan IPKs equivalentes para comparar tamaño y ABI. |
...
- Para personalizar el SDK durante la compilación basta pasar la versión/target como argumentos:
	...
	```
	Esto reutiliza el pipeline de descarga y sincronización pero apunta al `gcc` publicado junto con OpenWrt 23.05, lo que permite medir divergencias respecto al build predeterminado (24.10.5 `gcc-13.3`).
...
> ¿Buscas detalles adicionales sobre flujos internos, controles de seguridad y observabilidad? Revisa [`ARCHITECTURE.md`](ARCHITECTURE.md) para obtener un desglose actualizado.

> **Nota:** Todas las dependencias del daemon se instalan vía `opkg`, salvo las bibliotecas que solo existen en PyPI (`aiomqtt==2.4.0`, `paho-mqtt==2.1.0`, `pyserial-asyncio==0.6`, `cobs==1.2.2`, `tenacity==9.1.2`, `prometheus-client==0.23.1`, `persist-queue==1.1.0`, `attrs==24.2.0`). Esos paquetes se obtienen automáticamente de PyPI durante `3_install.sh`, que ahora lee `../requirements/runtime.toml` y aplica los mismos pines usados por `requirements/runtime.txt`. El resto (`python3-pyserial==3.5`, `python3-psutil==5.9.8`, `python3-more-itertools==10.8.0`, etc.) sigue llegando como IPKs tradicionales. El inventario completo vive en `../requirements/runtime.toml`; ejecuta `./tools/sync_runtime_deps.py` tras modificarlo para regenerar `requirements/runtime.txt`, refrescar el `Makefile` e instruir a `1_compile.sh`.
