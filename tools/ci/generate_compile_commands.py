...
cmd_str = (
    ...
    f"-I{ARD_LIBS}/Embedded_Template_Library/arduino -I{ARD_LIBS}/wolfssl "
    f"-I{ARD_LIBS}/wolfssl/src -I{ARD_LIBS}/wolfSSL "
    ...
    commands = [
        {
            ...
            "command": cmd_str.replace(
                "mcubridge-library-arduino/src/Bridge.cpp",
                f"{'-x c++-header ' if p.suffix == '.h' else ''}{p.relative_to(ROOT)}",
            ),
            "file": str(p),
        }
        for p in cpp_files
