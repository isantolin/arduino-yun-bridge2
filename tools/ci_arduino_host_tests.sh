...
# First-party sources (compiled with strict warnings)
SOURCES=(
...
# Third-party/autogenerated sources (compiled without strict warnings)
TP_SOURCES=(
...
# Strict flags for first-party code (no suppressions)
BASE_FLAGS=(
    -O2
    -g
    -Wall
...
# Relaxed flags for third-party/autogenerated code
TP_FLAGS=(
    -O2
    -g
    -DBRIDGE_HOST_TEST=1
...
# Compile first-party sources with strict flags in parallel
echo "[host-cpp] Compiling first-party sources in parallel..."
OBJECTS=()
...
# Compile third-party/autogenerated sources with relaxed flags in parallel
echo "[host-cpp] Compiling third-party sources in parallel..."
for src in "${TP_SOURCES[@]}"; do
    obj_name=$(basename "${src}")
    obj="${OBJ_DIR}/${obj_name}.o"
    OBJECTS+=("${obj}")
    ...
    if [[ "${src}" == *.c ]]; then
        ...
        g++ -std=c++17 "${TP_FLAGS[@]}" -c "${src}" -o "${obj}" &
    fi
done
wait
