...
This directory contains the **Source of Truth** for the Yun Bridge Protocol.
...
### 2. The Application Defaults (`yunbridge.const`)
...
- **Relation to Protocol:** It **does not** mirror protocol constants. Code requiring protocol constants (like Frame Delimiters or Command IDs) must import them directly from `yunbridge.rpc.protocol`.

### 3. The Runtime Configuration (`yunbridge.config.settings`)
- **Role:** Operational State.
- **Content:** Final values loaded from OpenWrt's UCI system (`/etc/config/yunbridge`), Environment Variables, or `yunbridge.const` defaults.
...
- **Protocol Constants:** Must be imported from `yunbridge.rpc.protocol`.
- **App Defaults:** Must be imported from `yunbridge.const`.
