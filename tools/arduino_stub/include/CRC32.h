...
// Minimal host-side stub for bakercp/CRC32 so the coverage harness can
// compile without the Arduino library installed. Implements the same
...
class CRC32 {
 public:
  ...
  template <typename Type>
  void update(const Type* data, size_t count) {
    if (!data && count > 0) {
      return;
    }
    const uint8_t* bytes = reinterpret_cast<const uint8_t*>(data);
    size_t total = count * sizeof(Type);
