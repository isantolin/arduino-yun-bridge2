...
@pytest.mark.asyncio
async def test_prometheus_exporter_serves_metrics(runtime_state, socket_enabled: None):
    ...
    assert b"mcubridge_mqtt_queue_limit" in payload
    assert b"mcubridge_file_storage_quota_bytes" in payload
    assert b"mcubridge_supervisors_worker_restarts" in payload
