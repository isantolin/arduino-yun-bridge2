...
UNITY_PATH="$ARDUINO_LIBS/Unity/src"
...
if [ ! -d "$UNITY_PATH" ]; then
    UNITY_PATH="$REPO_ROOT/.dummy_libs/Unity/src"
fi

CFLAGS="-std=c++17 -O0 -g -DBRIDGE_HOST_TEST=1 -DUNITY_INCLUDE_DOUBLE -DBRIDGE_ENABLE_SPI=1  -DWOLFSSL_USER_SETTINGS -DETL_NO_STL -Isrc -Isrc/config -Isrc/protocol -I$UNITY_PATH -I../tools/arduino_stub/include -I$ETL_PATH -I$WOLFSSL_PATH -I$PACKETSERIAL_PATH"
...
UNITY_OBJ="build-host-local/unity.o"
...
if [ ! -f $UNITY_OBJ ]; then
  gcc -O0 -g -c $UNITY_PATH/unity.c -o $UNITY_OBJ
fi

for t in $TESTS; do
  echo "=== Building $t ==="
  g++ $CFLAGS $SOURCES $WOLF_SOURCES tests/${t}.cpp $UNITY_OBJ -o build-host-local/${t} 2>&1
  echo "=== Running $t ==="
