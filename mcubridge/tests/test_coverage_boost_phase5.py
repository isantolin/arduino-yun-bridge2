...
def _make_config() -> RuntimeConfig:
    return RuntimeConfig(topic_prefix='br', serial_port='/dev/null', serial_baud=115200, cloud_spool_dir='/tmp/spool_test', allow_non_tmp_paths=True)
...
@pytest.mark.asyncio
async def test_local_bridge_service_publish_timeout_and_oserror(test_config: RuntimeConfig, mock_state: RuntimeState, mocker: MockerFixture) -> None:
    serial = AsyncMock(spec=SerialTransport)
    svc = BridgeService(test_config, mock_state, serial)
    local_svc = LocalBridgeService(svc)
    req_msg = pb.CloudQueuedPublish(topic_name='br/file/read', payload=b'test', correlation_data=b'corr-timeout-1')
...
@pytest.mark.asyncio
async def test_local_bridge_service_execute_rpc_unknown_method(test_config: RuntimeConfig, mock_state: RuntimeState) -> None:
    serial = AsyncMock(spec=SerialTransport)
    svc = BridgeService(test_config, mock_state, serial)
    local_svc = LocalBridgeService(svc)
...
@pytest.mark.asyncio
async def test_flush_cloud_spool_corrupt_and_index_error(test_config: RuntimeConfig, mock_state: RuntimeState, mocker: MockerFixture) -> None:
    serial = AsyncMock(spec=SerialTransport)
    svc = BridgeService(test_config, mock_state, serial)
...
def test_serial_transport_switch_local_baudrate_error(test_config: RuntimeConfig, mock_state: RuntimeState) -> None:
    ...
    type(mock_serial).baudrate = property(fget=lambda self: 115200, fset=MagicMock(side_effect=ValueError('Invalid baud')))
    transport.serial = mock_serial
...
@given(nonce=st.binary(min_size=1, max_size=32))
...
@settings(max_examples=25, derandomize=True, deadline=None)
@given(free_bytes=st.integers(0, 1024), data=st.binary(min_size=1, max_size=2048))
def test_runtime_write_with_quota_property(mocker: MockerFixture, tmp_path_factory: pytest.TempPathFactory, free_bytes: int, data: bytes) -> None:

    async def _run() -> None:
        tmp_dir = tmp_path_factory.mktemp('quota')
        config = RuntimeConfig(file_system_root=str(tmp_dir), cloud_spool_dir=str(tmp_dir / 'spool'), allow_non_tmp_paths=True)
...
@pytest.mark.asyncio
async def test_runtime_terminate_process_escalation(test_config: RuntimeConfig, mock_state: RuntimeState, mocker: MockerFixture) -> None:
    serial = AsyncMock(spec=SerialTransport)
    svc = BridgeService(test_config, mock_state, serial)
...
@settings(max_examples=25, derandomize=True, deadline=None)
@given(action=st.sampled_from(['read', 'write', 'mode', 'toggle']), topic_str=st.text(alphabet='abcdefghijklmnopqrstuvwxyz0123456789_', min_size=1, max_size=20))
def test_runtime_reject_cloud_topic_variants(test_config: RuntimeConfig, mock_state: RuntimeState, mocker: MockerFixture, action: str, topic_str: str) -> None:
...
@pytest.mark.asyncio
async def test_runtime_cleanup_and_lifecycle(test_config: RuntimeConfig, mock_state: RuntimeState, tmp_path: Path) -> None:
    serial = AsyncMock(spec=SerialTransport)
    svc = BridgeService(test_config, mock_state, serial)
