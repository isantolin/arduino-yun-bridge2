...
def test_structures_coverage_boost():
    ...
    caps = McuCapabilities(board_arch=1) # AVR
    assert "AVR" in caps.arch_name
    
    caps.board_arch = 2 # ESP32
    assert "ESP32" in caps.arch_name
    
    caps.board_arch = 99 # Unknown
    assert "Unknown" in caps.arch_name
