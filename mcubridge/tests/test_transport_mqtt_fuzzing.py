...
@pytest.mark.asyncio
async def test_mqtt_transport_retry_logic(mock_config, mock_state):
...
@pytest.mark.asyncio
async def test_mqtt_transport_exception_group_retry(mock_config, mock_state):
    transport = MqttTransport(mock_config, mock_state)
...
@pytest.mark.asyncio
async def test_mqtt_enqueue_without_client(mock_config, mock_state):
    transport = MqttTransport(mock_config, mock_state)
    ...
    mock_state.metrics.mqtt_messages_dropped.inc.assert_called()

@pytest.mark.asyncio
async def test_mqtt_subscriber_loop_error_handling(mock_config, mock_state):
    transport = MqttTransport(mock_config, mock_state)
