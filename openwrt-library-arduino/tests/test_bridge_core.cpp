...
public:
    static size_t build(uint8_t* out, size_t out_cap, uint16_t cmd_id, const uint8_t* payload, size_t payload_len) {
...
void sync_bridge(RecordingStream& stream) {
    ...
    if (ba.isSharedSecretEmpty()) {
        ...
        ba.assignSharedSecret(reinterpret_cast<const uint8_t*>(test_secret), reinterpret_cast<const uint8_t*>(test_secret) + strlen(test_secret));
    }
    ...
    const size_t frame_len = TestFrameBuilder::build(encoded_frame, sizeof(encoded_frame), rpc::to_underlying(rpc::CommandId::CMD_LINK_SYNC), payload, sizeof(payload));
...
void test_bridge_process_rx() {
    ...
    const size_t encoded_len = TestFrameBuilder::build(encoded_frame, 128, rpc::to_underlying(rpc::CommandId::CMD_GET_VERSION), nullptr, 0);
