...
@pytest.mark.asyncio
async def test_handshake_synchronize_retry_exhaustion(
...
@pytest.mark.asyncio
async def test_handle_link_sync_resp_auth_mismatch(
    handshake_mgr: SerialHandshakeManager,
):
    nonce = b"A" * 16
...
async def test_publish_handshake_event_extra(handshake_mgr: SerialHandshakeManager):
    handshake_mgr._state.handshake_fatal_count = 1
    await handshake_mgr.handle_handshake_failure("reason", detail="detail")
    cast(AsyncMock, handshake_mgr._enqueue_mqtt).assert_called()