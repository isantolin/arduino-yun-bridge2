...
def _make_config(**overrides: Any) -> RuntimeConfig:
    cfg = RuntimeConfig(serial_port='/dev/ttyMCU', serial_baud=115200, serial_safe_baud=9600, serial_shared_secret=b'testsharedsecret', allow_non_tmp_paths=True, allowed_commands=['echo', 'ls'])
...
class TestSerialRun:
    ...
    async def test_run_fatal_handshake(self, mocker: MockerFixture) -> None:
        ...
        state = _make_state(config)
        transport = SerialTransport(config, state, None)
        mocker.patch.object(transport, '_connect_and_run', new_callable=AsyncMock, side_effect=SerialHandshakeFatal('fatal'))
...
class TestSerialAcknowledge:
    ...
    @given(cmd=st.sampled_from([Command.CMD_GET_VERSION.value, Command.CMD_DIGITAL_WRITE.value, Command.CMD_ANALOG_WRITE.value]), seq=st.integers(0, 65535))
...
class TestLmdbDequeVacuum:
    ...
    async def test_vacuum_disk_failure(self) -> None:
        ...
        try:
            ...
            setattr(deque, 'env', mock_env)
            await deque.vacuum()
            setattr(deque, 'env', original_env)
            assert len(deque) >= 0
        finally:
            if deque is not None:
                await deque.close()
...
class TestLmdbCache:
    ...
    @settings(max_examples=25, derandomize=True, deadline=None)
    @given(key=st.text(alphabet='abcdefghijklmnopqrstuvwxyz0123456789_', min_size=1, max_size=32), value=st.binary(min_size=0, max_size=128), fallback=st.binary(min_size=0, max_size=32))
...
class TestRuntimeStateContext:
    ...
    def test_build_serial_pipeline_snapshot_with_data(self) -> None:
        config = _make_config()
        state = _make_state(config)
        state.serial_pipeline_inflight = {'event': 'send', 'command_id': 16, 'attempt': 1, 'ack_received': False, 'status': 0, 'timestamp': 1234567890.0}
        state.serial_pipeline_last = {'event': 'complete', 'command_id': 16, 'attempt': 1, 'ack_received': True, 'status': 1, 'timestamp': 1234567891.0}
...
class TestPinRestCgiCli:
    ...
    @settings(max_examples=25, derandomize=True, deadline=None)
    @given(pin=st.integers(0, 32), state_str=st.sampled_from(['ON', 'OFF']))
    ...
    @given(invalid_body=st.sampled_from([b'{"state": "INVALID_UNKNOWN_STATE"}', b'{"state": 9999}', b'{"value": "not_an_int"}', b'{"unknown_field": "val"}', b'not_json_at_all']))
    def test_application_pin_data_validation_error(self, mocker: MockerFixture, invalid_body: bytes) -> None:
        start_response = MagicMock()
        env = {'PATH_INFO': '/pin/13', 'REQUEST_METHOD': 'POST', 'CONTENT_LENGTH': str(len(invalid_body)), 'wsgi.input': BytesIO(invalid_body)}
        mocker.patch.object(pin_rest_cgi, 'load_runtime_config', return_value=_make_config())
        mocker.patch.object(pin_rest_cgi, 'configure_logging')
...
class TestRotateCredentials:
    ...
    def test_restart_service_failure(self, mocker: MockerFixture) -> None:
        import subprocess
        rotate_creds = _load_script('mcubridge_rotate_credentials')
        mock_run = mocker.patch.object(rotate_creds.subprocess, 'run', side_effect=subprocess.CalledProcessError(1, 'restart', stderr=b'failed'))
        rotate_creds.restart_service()
        mock_run.assert_called_once()
