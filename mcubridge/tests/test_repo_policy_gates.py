...
class _ShadowVisitor(ast.NodeVisitor):
    def __init__(self, path: Path, imported_names: set[str]) -> None:
        self.path = path
    ...
    def visit(self, node: ast.AST) -> typing.Any:
        method = getattr(self, f"_visit_{node.__class__.__name__.lower()}", None)
    ...
    def _visit_excepthandler(self, node: ast.ExceptHandler) -> None:
        if node.name:
            self._check_name(node.name, node.lineno, "except as")
        self.generic_visit(node)


def _find_shadowing_and_scope_escapes(
    py_files: list[Path],
) -> list[tuple[Path, int, str]]:
    ...
    for path in py_files:
        ...
        visitor = _ShadowVisitor(path, imported_names)
        visitor.visit(tree)
