...
async def perform_round_trip(
    ...
    @tenacity.retry(
        ...
        retry=tenacity.retry_if_exception_type(
            (asyncio.TimeoutError, aiomqtt.MqttError)
        ),
        reraise=True,
    )
    async def _round_trip() -> str:
        async with aiomqtt.Client(
            ...
            tls_context=(
                cast(ssl.SSLContext, tls_params["context"]) if tls_params else None
            ),
...
def main() -> None:
    ...
    if not serial_secret or serial_secret == "changeme123":
        sys.stderr.write(
            "[mcubridge-hw-smoke] ERROR: Serial secret missing or insecure.\n"
        )
    ...
    if init_script.exists():
        res = subprocess.run(
            [str(init_script), "status"], capture_output=True, check=False
        )
        if res.returncode != 0:
            sys.stderr.write(
                "[mcubridge-hw-smoke] ERROR: McuBridge service is not running.\n"
            )
            raise typer.Exit(code=1)

    if not STATUS_FILE.exists() or STATUS_FILE.stat().st_size == 0:
        sys.stderr.write(
            f"[mcubridge-hw-smoke] ERROR: Status file {STATUS_FILE} missing or empty.\n"
        )
        raise typer.Exit(code=1)
    ...
    if tls_enabled:
        ...
        tls_params = {"context": ctx, "insecure": insecure}

    # 4. Run Smoke Test
    sys.stderr.write(
        f"[mcubridge-hw-smoke] Checking MQTT round trip via {host}:{port}...\n"
    )

    try:
        response = asyncio.run(
            perform_round_trip(host, port, topic_prefix, tls_params, auth_params)
        )
        ...
        sys.stderr.write(
            "[mcubridge-hw-smoke] ERROR: Did not receive response from daemon (Timeout).\n"
        )
        raise typer.Exit(code=1)
