...
def _load_spec() -> (
    tuple[dict[str, int], list[_StatusSpec], list[_CommandSpec], dict[str, int | str]]
):
...
def test_handshake_tag_reference_vector_matches_spec() -> None:
    ...
    expected = hmac.new(auth_key, nonce, hashlib.sha256).digest()[
...
def test_mcu_inbound_commands_have_cpp_jump_table_handlers() -> None:
    ...
    header = (
        f"{len(missing)} MCU-inbound command(s) without C++ dispatch registration:\n"
    )
