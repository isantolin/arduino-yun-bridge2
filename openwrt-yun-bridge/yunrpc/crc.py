# yunrpc/crc.py
# CRC-16-CCITT calculation functions.
from typing import List

# Pre-calculated CRC-16-CCITT table for speed.
...
def crc16_ccitt(data: bytes, initial: int = 0xFFFF) -> int:
    ...
    Args:
        ...
        initial: The initial CRC value. Defaults to 0xFFFF.
    ...
    crc = initial
    for byte in data:
        # The index into the table is the XOR of the current byte and the
        # high byte of the current CRC value.
        table_index = (crc >> 8) ^ byte
        # The new CRC is the low byte of the old CRC shifted left by 8 bits,
        # XORed with the corresponding value from the lookup table.
...
if __name__ == "__main__":
    ...
    print(f"Test Case 1: Data='{test_data_1.decode()}'")
    print(f"  - Expected CRC: {expected_crc_1:04X}")
    print(f"  - Calculated CRC: {calculated_crc_1:04X}")
    assert calculated_crc_1 == expected_crc_1, "Test Case 1 Failed!"
    print("  - PASSED")
    print("-" * 20)

    test_data_2 = b"Hello, World!"
    expected_crc_2 = 0x67DA
    ...
    print(f"  - Calculated CRC: {calculated_crc_2:04X}")
    assert calculated_crc_2 == expected_crc_2, "Test Case 2 Failed!"
    print("  - PASSED")
    print("-" * 20)
    ...
    expected_crc_4 = 0x7C3E
    calculated_crc_4 = crc16_ccitt(test_data_4)
    print(f"Test Case 4: Data={list(test_data_4)}")
    print(f"  - Expected CRC: {expected_crc_4:04X}")
    print(f"  - Calculated CRC: {calculated_crc_4:04X}")
    assert calculated_crc_4 == expected_crc_4, "Test Case 4 Failed!"
    print("  - PASSED")
    print("-" * 20)
