...
class Frame:
    ...
    def parse(raw_frame_buffer: bytes) -> tuple[int, bytes]:
        ...
        if len(raw_frame_buffer) < protocol.MIN_FRAME_SIZE:
            raise ValueError(f"Incomplete frame: size {len(raw_frame_buffer)} is less than minimum {protocol.MIN_FRAME_SIZE}")
        ...
        if payload_len != actual_payload_len:
            raise ValueError(f"Payload length mismatch. Header says {payload_len}, but got {actual_payload_len}")
