...
from mcubridge.protocol.structures import FileWritePacket, FileReadPacket, FileRemovePacket
...
async def test_handle_write_quota_exceeded(file_comp: FileComponent) -> None:
    payload = msgspec.msgpack.encode(FileWritePacket(path="test.txt", data=b"A" * 10000000))
...
async def test_handle_read_response_decode_error(file_comp: FileComponent) -> None:
    ...
    file_comp._pending_mcu_read = MagicMock(future=fut)
...
async def test_handle_mcu_read_disabled(file_comp: FileComponent) -> None:
    file_comp._mcu_backend_enabled = False
    inbound = MagicMock()
    ok = await file_comp._handle_mcu_read(inbound, "mcu/test.txt")
    assert not ok
...
async def test_handle_mcu_remove_disabled(file_comp: FileComponent) -> None:
    file_comp._mcu_backend_enabled = False
    inbound = MagicMock()
    ok = await file_comp._handle_mcu_remove(inbound, "mcu/test.txt")
    assert not ok
...
async def test_handle_mcu_write_invalid_identifier(file_comp: FileComponent) -> None:
    inbound = MagicMock()
    ok = await file_comp._handle_mcu_write(inbound, "mcu/../invalid", b"")
    assert not ok
