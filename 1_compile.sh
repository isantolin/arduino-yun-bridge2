...
# Target: OpenWrt 25.12.5 (APK System)
#
...
# [CONFIG] Target Final OpenWrt 25.12.5
        ...
        if [ -f /etc/debian_version ]; then
            ...
            if [ ${#PKG_PREFIX[@]} -ne 0 ] || [ "$EUID" -eq 0 ]; then
                ...
                "${PKG_PREFIX[@]}" apt-get install -y \
                    build-essential python3 python3-pip python3-setuptools python3-wheel python3-build python3-hatchling python3-black \
            ...
            if [ ${#PKG_PREFIX[@]} -ne 0 ] || [ "$EUID" -eq 0 ]; then
                ...
                "${PKG_PREFIX[@]}" dnf install -y \
                    ...
                    python3 python3-pip python3-setuptools python3-wheel python3-build python3-hatchling python3-black \
...
check_python_module() {
    ...
    # Fallback to pip install if apt module fails
    python3 -m pip install "$module" >/dev/null 2>&1 && return 0
...
# ==============================================================================
# [FIX CRITICO] Breaking Kconfig Recursive Dependencies (SDK 25.12.5)
# ==============================================================================
...
if [ -x "$_SDK_PY" ]; then
    ...
    "$_SDK_PY" -m pip install --no-cache-dir --ignore-installed --prefix "$SDK_DIR/staging_dir/hostpkg" \
        "pdm-backend>=2.4.0" "maturin>=1.4" "Cython>=3.1" "setuptools-scm" "hatchling" "hatch-vcs" "setuptools-rust" "cffi" 2>&1 | tail -5
else
