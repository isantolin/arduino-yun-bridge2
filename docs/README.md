...
- **Resiliencia de Vistas LuCI y Fallback Atómico Frente a Sesiones RPC (v2.8.7)**: Mitigación del defecto upstream en la librería C `python-ubus` ante la inyección automática del parámetro `ubus_rpc_session` por `uhttpd-mod-ubus`. Las vistas LuCI (`status.js` y `capabilities.js`) incorporan validación estricta de respuestas y fallback automático hacia el snapshot atómico `/tmp/mcubridge_status.json` a través de la API nativa en C de `rpcd` (`fs.read`), garantizando telemetría en tiempo real sin bloqueos ni códigos de error crudos en la interfaz web.
- **Inspección Segura y No Bloqueante de Descriptores en Arranque (`S10boot`) (v2.8.7)**: Reemplazo integral de escaneos bloqueantes de contenido (`grep -l`) en `/proc/*/fd/*` por inspección directa de metadatos de enlaces simbólicos (`ls -l | awk`) en `mcubridge.init` y `99-mcubridge-disable-askconsole`. Elimina los bloqueos del script `S10boot` originados al intentar leer descriptores asociados a puertos serie bloqueantes (`/dev/ttyS1`, `/dev/ttyATH0`), garantizando la inicialización fluida de los servicios de red y de sistema.
- **Paridad y Robustez en Emulación Hardware QEMU / Malta MIPS (v2.8.7)**: Configuración obligatoria del parámetro `rw` en la línea de comando del kernel QEMU (`root=/dev/sda rw console=ttyS0`) para garantizar la persistencia de sockets, PIDs y configuraciones UCI. Se establecen pautas de sincronización temporal para emulación de hardware con convertidores USB-serie físicos frente a la ausencia de conmutación de línea DTR y la ventana acotada de seguridad SIL-2 (`SYNC_TIMEOUT_MS = 30000ms`).
- **Worker Thread Dedicado para UBUS y Cero Overhead de CPU (v2.8.7)**:
  - Desacoplamiento completo del bucle de eventos UBUS en un hilo de trabajo dedicado (`threading.Thread(name="ubus-worker", daemon=True)`) con llamadas bloqueantes en kernel (`ubus.loop(250)`). Las corutinas asíncronas se despachan thread-safe al bucle principal de asyncio mediante `asyncio.run_coroutine_threadsafe`, reduciendo el uso de CPU de 90% a 0% en reposo sobre procesadores embebidos mononúcleo (MIPS 24Kc).
- **Handler CGI POSIX de Ultra-Baja Latencia (`pin_rest_cgi.sh`) (v2.8.7)**:
  - Implementación de un handler POSIX ligero para `/www/cgi-bin/mcubridge-pin` que delega de forma directa a `ubus call mcubridge digital_write`. Elimina por completo el overhead de inicio de Python y la carga de dependencias pesadas, reduciendo la latencia de las peticiones REST de >30s a <500 ms (<50 ms tiempo de cómputo interno) con una huella de memoria inferior a 200 KB.
- **Paridad Universal de Sesiones LuCI JSON-RPC (`ubus_rpc_session`) (v2.8.7)**:
  - Registro automático del parámetro `ubus_rpc_session: STRING` en las firmas de todos los métodos de `UbusService`. Resuelve la incompatibilidad con `uhttpd-mod-ubus` y la estricta validación de argumentos de `python-ubus`, erradicando el error `UBUS_STATUS_INVALID_ARGUMENT = 2` en interfaces LuCI JavaScript.
- **Conducción Activa Hardware GPIO en Firmware MCU (v2.8.7)**:
  - Garantía en `BridgeClass::_handleDigitalWrite` de configuración `pinMode(pin, OUTPUT)` previa a la ejecución de `digitalWrite`, asegurando una conmutación lógica activa a 5V/3.3V y evitando que pines no inicializados activen resistencias de pull-up débiles.
- **Cobertura de Ramas C++ SIL-2 Elevada al 96.6% (v2.8.7)**:
  - Cobertura exhaustiva en la suite de pruebas unitarias (`test_bridge_edge_paths.cpp`), alcanzando un **96.6% de ramas** y **99.0% de líneas** validadas en el core del microcontrolador, superando con holgura el umbral de certificación SIL-2 (95.0%).
...
- **Consolidación de RPC en Cloud Gateway & Eliminación de Sockets UNIX (v2.8.5)**: Reemplazo integral de los sockets UNIX locales para clientes por el servicio gRPC `GatewayLocalBridgeService` en el Cloud Gateway centralizado, con resolución estrictamente explícita de dispositivo (`x-device-id`). Los clientes externos y herramientas CLI interactúan a través de gRPC tipado hacia el Gateway, mientras que el IPC local en OpenWrt Linux MPU se resuelve de forma nativa a través de UBUS (`ubus call mcubridge ...`).
...
### Detalles Técnicos del Stack gRPC & Arquitectura de Clientes (v2.8.5)
...
- **Northbound GatewayLocalBridgeService:** El Cloud Gateway expone los 22 métodos tipados de `LocalBridge` y streaming `SubscribeConsole`. Todos los clientes se conectan al Gateway indicando explícitamente el dispositivo de destino vía metadatos `x-device-id`. El Gateway despacha las operaciones hacia el daemon de borde correspondiente a través del stream de sesión activo.
- **Local IPC Nativo en OpenWrt:** En el Linux MPU de borde, los procesos del sistema, interfaces CGI y LuCI Web UI interactúan mediante llamadas nativas UBUS (`ubus call mcubridge ...`), eliminando sockets UNIX locales desprotegidos y reduciendo el consumo de RAM.
