...
4.  **Absolute Suppression Ban & Strict Encoding Integrity:** Suppressions of any kind are strictly prohibited; specifically, all warnings MUST be treated as errors, and "Pokemon" exceptions (catch-all) as well as silencing mechanisms such as errors="ignore", errors="replace", or errors="backslashreplace" on decode/encode are forbidden, with the sole exception of autogenerated code, which may be suppressed. If any other suppressions are found in the codebase, they must be removed immediately. All string/byte transformations must be strict UTF-8 to ensure deterministic failure on malformed input. Autogenerated files (e.g., protobuf/gRPC stubs) are strictly excluded from static verification checks (Pyright, Ruff).
