...
from mcubridge.protocol.structures import (
    ...
    ProcessRunAsyncPacket, ProcessKillPacket, ProcessPollPacket
)
...
async def test_runtime_brute_force_handlers_v2(service_setup: Any) -> None:
    ...
    # Call EVERY private handler with valid and invalid data
    handlers = [
        ...
        (service._handle_mcu_console_write, 1, msgspec.msgpack.encode(ConsoleWritePacket(data=b"test"))),
        (service._handle_mcu_datastore_put, 1, msgspec.msgpack.encode(DatastorePutPacket(key="k", value=b"v"))),
        (service._handle_mcu_datastore_get, 1, msgspec.msgpack.encode(DatastoreGetPacket(key="k"))),
        (service._handle_mcu_mailbox_push, 1, msgspec.msgpack.encode(MailboxPushPacket(data=b"m"))),
        (service._handle_mcu_mailbox_read, 1, b""),
        (service._handle_mcu_mailbox_available, 1, b""),
        (service._handle_mcu_mailbox_processed, 1, msgspec.msgpack.encode(MailboxProcessedPacket(message_id=1))),
        (service._handle_mcu_file_write, 1, msgspec.msgpack.encode(FileWritePacket(path="f", data=b""))),
        (service._handle_mcu_file_read, 1, msgspec.msgpack.encode(FileReadPacket(path="f"))),
        (service._handle_mcu_file_remove, 1, msgspec.msgpack.encode(FileRemovePacket(path="f"))),
        (service._handle_mcu_file_read_resp, 1, b"\x81\xa4data\xa4abc"),
        (service._handle_mcu_process_run, 1, msgspec.msgpack.encode(ProcessRunAsyncPacket(command="ls"))),
        (service._handle_mcu_process_poll, 1, msgspec.msgpack.encode(ProcessPollPacket(pid=1))),
        (service._handle_mcu_process_kill, 1, msgspec.msgpack.encode(ProcessKillPacket(pid=1))),
        (service._handle_mcu_pin_digital_read, 1, msgspec.msgpack.encode(PinReadPacket(pin=1))),
        (service._handle_mcu_pin_analog_read, 1, msgspec.msgpack.encode(PinReadPacket(pin=1))),
        (service._handle_mcu_pin_digital_read_resp, 1, msgspec.msgpack.encode(DigitalReadResponsePacket(value=1))),
        (service._handle_mcu_pin_analog_read_resp, 1, msgspec.msgpack.encode(AnalogReadResponsePacket(value=1))),
