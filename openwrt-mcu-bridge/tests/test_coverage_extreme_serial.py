...
async def test_serial_binary_packet_all_paths():
    # Covers line 50 and Sized checks
...
async def test_serial_protocol_connection_lost_full():
    ...
    # Line 91-98: connection_lost with and without exc
...
async def test_serial_protocol_data_received_complex():
    ...
    # Line 115-118: Transition to discarding
    ...
    # Line 151-158: _process_chunk_fast remnants
...
async def test_serial_process_packet_negotiation_full():
    ...
    # Malformed during negotiation (Line 173-176)
    proto.negotiation_future = asyncio.get_running_loop().create_future()
    proto._process_packet(b"\x00") # Malformed
...
async def test_serial_write_frame_full_coverage():
    ...
    # Line 245-246: No payload debug log
    ...
    # Line 254: Exception log
    proto.transport.write.side_effect = ValueError("fail")
    proto.write_frame(Command.CMD_GET_VERSION, b"")

@pytest.mark.asyncio
async def test_serial_transport_init_coverage():
    # Line 257-259: Just init
    serial_fast.SerialTransport(MagicMock(), MagicMock(), AsyncMock())
...
async def test_serial_transport_run_and_connect_failures():
    ...
    # Line 281-289: Handshake Fatal and Stop event
...
async def test_serial_transport_disconnect_hook_coverage():
    ...
    with patch("mcubridge.transport.serial_fast.serial_asyncio_fast.create_serial_connection", return_value=(m_t, MagicMock())):
        await transport._connect_and_run(asyncio.get_running_loop())
