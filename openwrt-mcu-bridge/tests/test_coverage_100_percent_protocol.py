...
def test_frame_parse_coverage_all_errors():
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        with pytest.raises(
            ValueError, match="(Incomplete header|Frame parsing failed)"
        ):
            Frame.parse(b"1234567")

    # Line 125: CRC Mismatch
    bad_crc_frame = structures.CRC_COVERED_HEADER_STRUCT.build(
        dict(version=protocol.PROTOCOL_VERSION, payload_len=0, command_id=0x40)
    )
    ...
    bad_ver = structures.CRC_COVERED_HEADER_STRUCT.build(
        dict(version=255, payload_len=0, command_id=0x40)
    )
    with pytest.raises(ValueError, match="Invalid version"):
        Frame.parse(_build_raw_with_crc(bad_ver))

    # Line 157: Payload length mismatch
    bad_len = structures.CRC_COVERED_HEADER_STRUCT.build(
        dict(version=protocol.PROTOCOL_VERSION, payload_len=10, command_id=0x40)
    )
