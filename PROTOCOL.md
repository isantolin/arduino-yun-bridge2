...
| `0x01` | `STATUS_ERROR`     | Fallo genérico.                                    |
...
- **`0x62` CMD_PROCESS_POLL (MCU → Linux)**
  ...
  - Respuesta `0xB2 CMD_PROCESS_POLL_RESP (Linux → MCU)`: `[status: u8, exit_code: u8, stdout_len: u16, stdout: byte[], stderr_len: u16, stderr: byte[]]`. `exit_code = 0xFF` cuando el proceso sigue en ejecución.
