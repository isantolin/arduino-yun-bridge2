...
async def test_datastore_handle_get_malformed() -> None:
    ...
    try:
        ...
        assert result is False
        serial_flow.send.assert_called_once_with(Status.MALFORMED.value, b"data_get_malformed")
        assert not mqtt_flow.publish.called
    finally:
        state.cleanup()
...
async def test_datastore_handle_mqtt_edge_cases() -> None:
    ...
    try:
        ...
        await comp.handle_mqtt(
            make_route(Topic.DATASTORE, DatastoreAction.GET.value, "echo_key"), make_mqtt_msg(b"val")
        )
        assert not mqtt_flow.publish.called
        ...
        await comp.handle_mqtt(
            make_route(Topic.DATASTORE, DatastoreAction.GET.value, "int_key", "request"), make_mqtt_msg(b"")
        )
...
async def test_datastore_mqtt_put_too_large() -> None:
    ...
    try:
        ...
        # Key too large
        long_key = "k" * 300
        await comp.handle_mqtt(make_route(Topic.DATASTORE, DatastoreAction.PUT.value, long_key), make_mqtt_msg(b"val"))
        assert not mqtt_flow.publish.called
        assert long_key not in state.datastore

        # Value too large
        long_val = b"v" * 300
        await comp.handle_mqtt(make_route(Topic.DATASTORE, DatastoreAction.PUT.value, "key"), make_mqtt_msg(long_val))
        assert not mqtt_flow.publish.called
...
async def test_datastore_mqtt_get_too_large() -> None:
    ...
    try:
        ...
        long_key = "k" * 300
        await comp.handle_mqtt(
            make_route(Topic.DATASTORE, DatastoreAction.GET.value, long_key, "request"), make_mqtt_msg(b"")
        )
        assert not mqtt_flow.publish.called
