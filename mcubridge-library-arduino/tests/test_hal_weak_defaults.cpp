...
void test_hal_weak_defaults_without_mock() {
  ...
  const auto write_res = bridge::hal::writeFile("test.txt", etl::span<const uint8_t>());
  ...
  uint8_t buffer[8];
  const auto read_res = bridge::hal::readFileChunk("test.txt", 0, etl::span<uint8_t>(buffer, 8));
