...
@pytest.mark.asyncio
async def test_publish_metrics_none_spool(mock_state: RuntimeState, mocker: MockerFixture) -> None:
    mock_enqueue = AsyncMock()
    mocker.patch.object(mock_state, "build_metrics_snapshot")
    mocker.patch("mcubridge.metrics._build_metrics_message", return_value=pb.CloudQueuedPublish())

    await publish_metrics(mock_state, mock_enqueue, spool=None)
...
async def test_handshake_publish_event_empty_topic(
    ...
    hm = SerialHandshakeManager(
        test_config, mock_state, AsyncMock(), mock_enqueue
    )
...
def test_run_daemon_keyboard_interrupt(mocker: MockerFixture) -> None:
