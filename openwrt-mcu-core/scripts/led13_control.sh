...
TOPIC_PREFIX=$(uci -q get mcubridge.general.mqtt_topic 2>/dev/null || printf '%s' "br")
MQTT_TOPIC=${TOPIC_PREFIX}/d/${PIN}

MQTT_HOST=$(uci -q get mcubridge.general.mqtt_host 2>/dev/null || printf '%s' "127.0.0.1")
MQTT_PORT=$(uci -q get mcubridge.general.mqtt_port 2>/dev/null || printf '%s' "8883")
MQTT_USER=$(uci -q get mcubridge.general.mqtt_user 2>/dev/null || printf '%s' "")
MQTT_PASS=$(uci -q get mcubridge.general.mqtt_pass 2>/dev/null || printf '%s' "")
MQTT_TLS=$(uci -q get mcubridge.general.mqtt_tls 2>/dev/null || printf '%s' "1")
MQTT_TLS_INSECURE=$(uci -q get mcubridge.general.mqtt_tls_insecure 2>/dev/null || printf '%s' "0")
MQTT_CAFILE=$(uci -q get mcubridge.general.mqtt_cafile 2>/dev/null || printf '%s' "")
