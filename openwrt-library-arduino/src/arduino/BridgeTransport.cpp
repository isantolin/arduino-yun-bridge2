...
bool BridgeTransport::processInput(rpc::Frame& rxFrame) {
  ...
  // [PROTOCOL 10/10] Anti-Starvation Loop
  // Procesamos solo un lote limitado de bytes para garantizar que
  // el loop() principal del usuario siempre tenga ciclos de CPU disponibles.
  int bytes_processed = 0;

  while (_stream.available() && bytes_processed < kMaxBytesPerYield) {
    ...
    if (byte_read >= 0) {
      ...
      if (_parser.overflowed()) {
        // We don't return true here, but the caller might want to know about
        // the error For now, we just let the parser state reflect it.
        return false;
      }
    }
  }
...
bool BridgeTransport::sendFrame(uint16_t command_id, const uint8_t* payload,
                                size_t length) {
  size_t raw_len =
      _builder.build(_raw_frame_buffer.data(), _raw_frame_buffer.size(),
                     command_id, payload, length);
  ...
  size_t cobs_len =
      cobs::encode(_raw_frame_buffer.data(), raw_len, _last_cobs_frame.data());
...
bool BridgeTransport::sendControlFrame(uint16_t command_id) {
  ...
  size_t raw_len =
      builder.build(raw_buf, sizeof(raw_buf), command_id, nullptr, 0);
