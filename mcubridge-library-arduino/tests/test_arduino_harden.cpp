...
void test_bridge_protocol_version_mismatch() {
    ...
    rpc::Frame f = {};
    f.header = {static_cast<uint8_t>(rpc::PROTOCOL_VERSION + 1), 0, static_cast<uint16_t>(rpc::CommandId::CMD_GET_VERSION), 1};
    f.nonce.fill(0);
    f.tag.fill(0);
    f.payload = {};
    ...
    TEST_ASSERT_EQUAL(0, stream.available()); // No response if rejected early usually
}
...
void test_bridge_tx_queue_full_force() {
    ...
    // Fill the queue
    etl::counter_iterator<int> fill_begin(0);
    etl::counter_iterator<int> fill_end(bridge::config::TX_QUEUE_CAPACITY);
...
void test_bridge_packet_received_edge_cases() {
    ...
    // 2. CRC mismatch
    rpc::Frame f = {};
    f.header = {rpc::PROTOCOL_VERSION, 0, static_cast<uint16_t>(rpc::CommandId::CMD_GET_VERSION), 1};
    f.nonce.fill(0);
    f.tag.fill(0);
    f.payload = {};
    f.crc = 0xDEADC0DE; 
...
void test_bridge_ack_orphans() {
    ...
    ba.handleAck(static_cast<uint16_t>(rpc::CommandId::CMD_GET_VERSION)); // Wrong ID
...
void test_bridge_linksync_auth_failure() {
    ...
    sync_msg.tag.fill(0xFF); // Wrong tag
...
void test_bridge_decompress_error() {
    ...
    f.header = {rpc::PROTOCOL_VERSION, 4, static_cast<uint16_t>(static_cast<uint16_t>(rpc::CommandId::CMD_GET_VERSION) | rpc::RPC_CMD_FLAG_COMPRESSED), 1};
...
void test_bridge_security_pre_sync_rejection() {
    ...
    // Should have emitted STATUS_ERROR (which is excluded from sync check usually)
    // but the restricted command itself was rejected.
    TEST_ASSERT_FALSE(ba.isSynchronized());
}
...
void test_bridge_nonce_reuse_attack() {
    ...
    rpc::payload::LinkSync sync_msg = {};
    sync_msg.nonce.fill(0xAA);
    ba.computeHandshakeTag(sync_msg.nonce.data(), 16, sync_msg.tag.data());
    ...
    // 2. Send frame with nonce counter = 1 (OK)
    // 3. Send frame with nonce counter = 1 again (Replay/Reuse attack)
    // This is harder to test without a full AEAD mock or real crypto
    // But we covered the counter check branch in _handleReceivedFrame.
}
