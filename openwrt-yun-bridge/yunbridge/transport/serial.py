...
async def _negotiate_baudrate(
    ...
    try:
        ...
        if resp_frame.command_id == Command.CMD_SET_BAUDRATE_RESP:
            ...
            logger.warning("Unexpected response during baudrate negotiation: 0x%02X", resp_frame.command_id)
            return False
...
class SerialTransport:
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.critical("Unhandled exception in SerialTransport.run", exc_info=True)
            finally:
                await self._disconnect()

            if should_retry:
                logger.warning("Serial port disconnected. Retrying in %d seconds...", reconnect_delay)
        ...
        while True:
            ...
            if byte == FRAME_DELIMITER:
                ...
                if len(buffer) > MAX_SERIAL_PACKET_BYTES:
                    ...
                    logger.warning("Serial packet exceeded %d bytes; requesting retransmit.", MAX_SERIAL_PACKET_BYTES)
                    ...
                    try:
                        ...
                        logger.exception("Failed to notify MCU about oversized serial packet")

    async def send_frame(self, command_id: int, payload: bytes) -> bool:
        if self.writer is None or self.writer.is_closing():
            logger.error("Serial writer closed; cannot send frame 0x%02X", command_id)
            return False
        ...
        try:
            ...
            logger.error("Serial connection reset while sending frame 0x%02X", command_id)
            await self._disconnect()
            return False
        except Exception:
    ...
    async def _process_packet(self, encoded_packet: object) -> None:
        if not _is_binary_packet(encoded_packet):
            logger.warning("Dropping non-binary serial packet type %s", type(encoded_packet).__name__)
            self.state.record_serial_decode_error()
        ...
        try:
            ...
            logger.warning("COBS decode error %s (len=%d)", exc, len(packet_bytes))
            self.state.record_serial_decode_error()
            truncated = packet_bytes[:32]
            payload = struct.pack(protocol.UINT16_FORMAT, protocol.INVALID_ID_SENTINEL) + truncated
            try:
                await self.service.send_frame(Status.MALFORMED.value, payload)
            except Exception:
        ...
        try:
            ...
            logger.warning("Frame parse error %s (len=%d)", exc, len(raw_frame))
