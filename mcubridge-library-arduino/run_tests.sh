...
if [ ! -d "$ETL_PATH" ]; then
    ETL_PATH="$REPO_ROOT/.dummy_libs/Embedded_Template_Library/include"
fi
...
CFLAGS="-std=c++17 -O0 -g -DBRIDGE_HOST_TEST=1 -DUNITY_INCLUDE_DOUBLE -DBRIDGE_ENABLE_SPI=1  -DWOLFSSL_USER_SETTINGS -DETL_NO_STL -Isrc -Isrc/config -Isrc/protocol -Itests/Unity/src -I../tools/arduino_stub/include -I$ETL_PATH -I$WOLFSSL_PATH -I$PACKETSERIAL_PATH -I$MPACK_PATH/src"
...
MPACK_SOURCES="$MPACK_PATH/src/mpack/mpack-common.c $MPACK_PATH/src/mpack/mpack-writer.c $MPACK_PATH/src/mpack/mpack-reader.c $MPACK_PATH/src/mpack/mpack-expect.c $MPACK_PATH/src/mpack/mpack-node.c $MPACK_PATH/src/mpack/mpack-platform.c"
