...
void test_surgical_bridge_errors() {
    ...
    f.header.command_id = static_cast<uint16_t>(rpc::CommandId::CMD_LINK_SYNC);
    f.header.sequence_id = 1;
    f.header.payload_length = 32;
...
void test_surgical_security_failures() {
    ...
    bool ok = rpc::security::aead_encrypt(
        etl::span<uint8_t>(out, 1),
        etl::span<uint8_t>(tag, 16),
        etl::span<const uint8_t>(data, 4),
        etl::span<const uint8_t>(key, 32),
        etl::span<const uint8_t>(nonce, 12),
        etl::span<const uint8_t>(ad, 12)
    );
    TEST_ASSERT_FALSE(ok);

    // aead_decrypt with mismatching tag
    ok = rpc::security::aead_decrypt(
        etl::span<uint8_t>(out, 4),
        etl::span<const uint8_t>(data, 4),
        etl::span<const uint8_t>(tag, 16),
        etl::span<const uint8_t>(key, 32),
        etl::span<const uint8_t>(nonce, 12),
        etl::span<const uint8_t>(ad, 12)
    );
