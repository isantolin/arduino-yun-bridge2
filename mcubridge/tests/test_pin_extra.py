...
async def test_pin_handle_read_overflow() -> None:
    ...
    try:
        ...
        result = await comp.handle_mqtt(route, make_inbound_message("test/topic", b""))
...
async def test_pin_handle_mqtt_edge_cases() -> None:
    ...
    try:
        ...
        await comp.handle_mqtt(route1, make_inbound_message("test/topic", b""))
        serial_flow.send.assert_not_called()

        # 2. Invalid pin
        route2 = TopicRoute("br/d/invalid", "br", Topic.DIGITAL, ("invalid",))
        await comp.handle_mqtt(route2, make_inbound_message("test/topic", b""))
        serial_flow.send.assert_not_called()

        # 3. Unknown subtopic
        route3 = TopicRoute("br/d/13/magic", "br", Topic.DIGITAL, ("13", "magic"))
        await comp.handle_mqtt(route3, make_inbound_message("test/topic", b""))
        serial_flow.send.assert_not_called()

        # 4. Invalid mode
        route4 = TopicRoute("br/d/13/mode", "br", Topic.DIGITAL, ("13", "mode"))
        await comp.handle_mqtt(route4, make_inbound_message("test/topic", b"invalid"))
        serial_flow.send.assert_not_called()

        await comp.handle_mqtt(route4, make_inbound_message("test/topic", b"99"))
        serial_flow.send.assert_not_called()

        # 5. Invalid write value
        route5 = TopicRoute("br/d/13", "br", Topic.DIGITAL, ("13",))
        await comp.handle_mqtt(route5, make_inbound_message("test/topic", b"invalid"))
        serial_flow.send.assert_not_called()
