...
@pytest.mark.asyncio
async def test_daemon_uvloop_missing():
    with patch("mcubridge.daemon.uvloop", None):
        with pytest.raises(SystemExit) as excinfo:
            main(["--serial-port", "/dev/ttyUSB0"])
        assert excinfo.value.code == 1

@pytest.mark.asyncio
async def test_daemon_crypto_failure():
...
async def test_mailbox_handle_processed_malformed():
    ...
    assert mailbox.mqtt_flow.enqueue_mqtt.called
...
async def test_mailbox_handle_push_overflow():
    state = MagicMock()
    state.mailbox_incoming_queue = [b"old"]
...
async def test_mailbox_handle_read_empty():
    ...
    args = mailbox.serial_flow.send.call_args
...
async def test_spi_handle_mqtt_malformed_config():
    spi = SpiComponent(MagicMock(), MagicMock(), AsyncMock(), AsyncMock())
    route = TopicRoute(prefix="br", topic="spi", identifier="config")
    msg = MagicMock()
...
async def test_file_handle_read_not_found():
    ...
    with patch.object(file_svc, "_get_safe_path", return_value=None):
        ...
        await file_svc.handle_read(1, msgspec.msgpack.encode(FileReadPacket(path="foo")))
        assert file_svc.serial_flow.send.called
...
async def test_file_handle_remove_fail():
    ...
    with patch.object(file_svc, "_get_safe_path", return_value=None):
        ...
        res = await file_svc.handle_remove(1, msgspec.msgpack.encode(FileRemovePacket(path="foo")))
        assert res is False
