...
class BridgeService:
    ...
    def __init__(self, config: RuntimeConfig, state: RuntimeState, serial: SerialTransport) -> None:
        ...
        self.mcu_registry = self._setup_mcu_registry()
        for s in Status:
            if s != Status.ACK:
    ...
    def _setup_mcu_registry(self) -> dict[int, McuHandler]:
        return {
            ...
            Command.CMD_CONSOLE_WRITE.value: self._gen_handler(ConsoleWritePacket, self._handle_mcu_console_write),
            Command.CMD_DATASTORE_PUT.value: self._gen_handler(DatastorePutPacket, self._handle_mcu_datastore_put),
            Command.CMD_DATASTORE_GET.value: self._gen_handler(DatastoreGetPacket, self._handle_mcu_datastore_get),
            Command.CMD_MAILBOX_PUSH.value: self._gen_handler(MailboxPushPacket, self._handle_mcu_mailbox_push),
            Command.CMD_MAILBOX_AVAILABLE.value: lambda seq, _: self._handle_mcu_mailbox_available(seq, b""),
            Command.CMD_MAILBOX_READ.value: lambda seq, _: self._handle_mcu_mailbox_read(seq, b""),
            Command.CMD_MAILBOX_PROCESSED.value: lambda _, p: self._handle_mcu_mailbox_processed(0, p),
            Command.CMD_FILE_WRITE.value: self._gen_handler(FileWritePacket, self._handle_mcu_file_write),
            Command.CMD_FILE_READ.value: self._gen_handler(FileReadPacket, self._handle_mcu_file_read),
            Command.CMD_FILE_REMOVE.value: self._gen_handler(FileRemovePacket, self._handle_mcu_file_remove),
            Command.CMD_FILE_READ_RESP.value: self._gen_handler(
                FileReadResponsePacket, self._handle_mcu_file_read_resp
            ),
            Command.CMD_PROCESS_RUN_ASYNC.value: self._gen_handler(ProcessRunAsyncPacket, self._handle_mcu_process_run),
            Command.CMD_PROCESS_POLL.value: self._gen_handler(ProcessPollPacket, self._handle_mcu_process_poll),
            Command.CMD_PROCESS_KILL.value: self._gen_handler(ProcessKillPacket, self._handle_mcu_process_kill),
            ...
            Command.CMD_DIGITAL_READ_RESP.value: self._gen_handler(
                DigitalReadResponsePacket,
                lambda p: self._handle_mcu_pin_resp(p, Topic.DIGITAL, self.state.pending_digital_reads),
            ),
            Command.CMD_ANALOG_READ_RESP.value: self._gen_handler(
                AnalogReadResponsePacket,
                lambda p: self._handle_mcu_pin_resp(p, Topic.ANALOG, self.state.pending_analog_reads),
            ),
            Command.CMD_SPI_TRANSFER_RESP.value: self._gen_handler(
                SpiTransferResponsePacket, self._handle_mcu_spi_resp
            ),
    ...
    async def handle_mqtt_message(self, inbound: Message) -> None:
        if route := parse_topic(self.state.mqtt_topic_prefix, str(inbound.topic)):
            ...
            if action and not (
                ...
                return
            dispatch = {
                Topic.CONSOLE: lambda: self._handle_mqtt_console(inbound),
                Topic.DATASTORE: lambda: self._handle_mqtt_datastore(route, inbound),
                Topic.MAILBOX: lambda: self._handle_mqtt_mailbox(route, inbound),
                Topic.FILE: lambda: self._handle_mqtt_file(route, inbound),
                Topic.SHELL: lambda: self._handle_mqtt_shell(route, inbound),
                Topic.SPI: lambda: self._handle_mqtt_spi(route, inbound),
                Topic.DIGITAL: lambda: self._handle_mqtt_pin(route, inbound),
                Topic.ANALOG: lambda: self._handle_mqtt_pin(route, inbound),
                Topic.SYSTEM: lambda: self._handle_mqtt_system(route, inbound),
            }
            if handler := dispatch.get(route.topic):
                await handler()

    # --- Business Logic Implementation ---

    async def _set_mcu_paused(self, paused: bool) -> None:
        self.state.mcu_is_paused = paused
        self.state.serial_tx_allowed.set() if not paused else self.state.serial_tx_allowed.clear()
        if not paused:
            await self._flush_console_queue()

    async def _handle_mcu_xon(self, seq_id: int, payload: bytes) -> None:
        await self._set_mcu_paused(False)

    async def _handle_mcu_xoff(self, seq_id: int, payload: bytes) -> None:
        await self._set_mcu_paused(True)

    async def _handle_mcu_console_write(self, p: ConsoleWritePacket) -> None:
        if p.data:
    ...
    async def _handle_mcu_datastore_put(self, p: DatastorePutPacket) -> None:
    ...
    async def _handle_mcu_datastore_get(self, p: DatastoreGetPacket) -> bool:
    ...
    async def _handle_mcu_mailbox_push(self, p: MailboxPushPacket) -> None:
    ...
    async def _handle_mcu_mailbox_available(self, seq: int, _: bytes) -> bool:
        return await self.serial.send(
    ...
    async def _handle_mcu_mailbox_read(self, seq: int, _: bytes) -> bool:
        return await self.serial.send(
    ...
    async def _handle_mcu_mailbox_processed(self, seq: int, payload: bytes) -> None:
        await self.enqueue_mqtt(
            QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.MAILBOX, MailboxAction.PROCESSED), payload)
        )

    async def _handle_mcu_file_write(self, p: FileWritePacket) -> bool:
    ...
    async def _handle_mcu_file_read(self, p: FileReadPacket) -> None:
        path = self._get_safe_path(p.path)
    ...
    async def _handle_mcu_file_remove(self, p: FileRemovePacket) -> bool:
        path = self._get_safe_path(p.path)
    ...
    async def _handle_mcu_file_read_resp(self, p: FileReadResponsePacket) -> bool:
    ...
    async def _handle_mcu_process_run(self, p: ProcessRunAsyncPacket) -> bool:
        ...
        return await self.serial.send(Status.ERROR.value, b"Exec failed")

    async def _handle_mcu_process_poll(self, p: ProcessPollPacket) -> bool:
    ...
    async def _handle_mcu_process_kill(self, p: ProcessKillPacket) -> None:
    ...
    async def _handle_mcu_spi_resp(self, p: SpiTransferResponsePacket) -> None:
        await self.enqueue_mqtt(
            QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SPI, "transfer", "resp"), p.data)
        )

    async def _handle_mcu_ack(self, seq: int, payload: bytes) -> None:
