...
def test_serial_ensure_raw_mode_no_fd() -> None:
    ...
    from yunbridge.transport.serial import _ensure_raw_mode
...
async def test_emit_metrics_snapshot_exception_on_initial() -> None:
    ...
    from yunbridge.metrics import _emit_metrics_snapshot
...
async def test_prometheus_exporter_get_root() -> None:
    ...
    try:
        ...
        assert b"yunbridge" in response  # Should contain metrics
        writer.close()
        await writer.wait_closed()
    finally:
        await exporter.stop()
