...
- **Límites configurables:** Los buffers internos de mailbox se pueden ajustar vía UCI (`mailbox_queue_limit`, `mailbox_queue_bytes_limit`) para prevenir desbordes. Se incluyen límites estrictos como `pending_pin_request_limit`, `file_write_max_bytes` y `file_storage_quota_bytes`.
...
- Revisión manual de los bindings regenerados ejecutando `led13_test.py` y `datastore_test.py` del paquete `openwrt-mcu-examples-python`, confirmando compatibilidad funcional.
