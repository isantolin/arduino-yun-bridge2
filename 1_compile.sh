...
if [ -x "$_SDK_PY" ]; then
    ...
    "$_SDK_PY" -m pip install --no-cache-dir --ignore-installed --prefix "$SDK_DIR/staging_dir/hostpkg" \
        "pdm-backend>=2.4.0" "maturin>=1.4" "Cython>=3.1" "setuptools-scm[simple]>=8" "packaging>=26.2" "vcs-versioning" "distlib>=0.3.8" "hatchling" "hatch-vcs" "setuptools-rust" "cffi" 2>&1 | tail -5
else
