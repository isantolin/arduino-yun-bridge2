#!/usr/bin/env python3
"""Audit codebase for SIL-2/MIL-SPEC violations, Pokemon exceptions, and shims."""
...
def audit_cpp_files() -> list[str]:
    ...
    for cpp_file in cpp_dir.rglob("*"):
        if cpp_file.suffix not in (".h", ".cpp"):
            continue
        if any(skip in cpp_file.name for skip in ("mcubridge.pb", "pb_encode", "pb_decode", "pb_common")):
            continue
        ...
        for i, line in enumerate(content.splitlines(), 1):
            ...
            if loop_pattern.search(line):
                findings.append(f"C++ Loop Found: {cpp_file.name}:{i} - '{line.strip()}'")
            if non_template_wrapper_pattern.search(line):
                findings.append(f"C++ Non-Template Wrapper Found: {cpp_file.name}:{i} - '{line.strip()}'")
    return findings
