...
class BridgeService:
    ...
    async def enqueue_mqtt(self, message: QueuedPublish, *, reply_context: Message | None = None) -> None:
        ...
        if msg.payload_format_indicator is not None: props.PayloadFormatIndicator = msg.payload_format_indicator
        if msg.message_expiry_interval: props.MessageExpiryInterval = msg.message_expiry_interval
        ...
        try:
            ...
            logger.warning("MQTT publish failure: %s", exc); self.state.mqtt_dropped_messages += 1

    # --- Lifecycle ---

    async def __aenter__(self) -> BridgeService:
        self._task_group = asyncio.TaskGroup(); await self._task_group.__aenter__(); return self

    async def __aexit__(self, et: Any, ev: Any, tb: Any) -> None:
        if self._task_group: await self._task_group.__aexit__(et, ev, tb)

    async def on_serial_connected(self) -> None:
        self.state.mark_transport_connected(); await self.handshake.synchronize()
        if self.state.is_synchronized: await self._request_mcu_version(); await self._flush_console_queue()

    async def on_serial_disconnected(self) -> None:
        self.state.mark_transport_disconnected()
        for q in (self.state.pending_digital_reads, self.state.pending_analog_reads): q.clear()
        self.state.mcu_is_paused = False; self.state.serial_tx_allowed.set(); self.handshake.clear_handshake_expectations(); await self.serial.reset()

    # --- Dispatchers ---

    async def handle_mcu_frame(self, cmd_id: int, seq_id: int, payload: bytes) -> None:
        if not (self.state.is_synchronized or cmd_id in _STATUS_VALUES or cmd_id in _PRE_SYNC_ALLOWED_COMMANDS): return
        if handler := self.mcu_registry.get(cmd_id):
            if await handler(seq_id, payload) is not False and cmd_id not in _STATUS_VALUES: await self.serial.acknowledge(cmd_id, seq_id)
        elif response_to_request(cmd_id) is None: self.state.metrics.unknown_command_count.inc(); await self.serial.send(Status.NOT_IMPLEMENTED.value, b"")
        ...
        if route := parse_topic(self.state.mqtt_topic_prefix, str(inbound.topic)):
            if route.topic != Topic.SYSTEM:
                with contextlib.suppress(asyncio.TimeoutError):
                    async with asyncio.timeout(30.0): await self.state.link_sync_event.wait()
            action = self._deduce_action(route)
            if action and not (self.state.topic_authorization.allows(route.topic.value if isinstance(route.topic, Topic) else route.topic, action) if self.state.topic_authorization else False):
                await self._reject_mqtt(inbound, route.topic, action); return
            
            # Unified MQTT Dispatch
            match route.topic:
                case Topic.CONSOLE: await self._handle_mqtt_console(inbound)
                case Topic.DATASTORE: await self._handle_mqtt_datastore(route, inbound)
                case Topic.MAILBOX: await self._handle_mqtt_mailbox(route, inbound)
                case Topic.FILE: await self._handle_mqtt_file(route, inbound)
                case Topic.SHELL: await self._handle_mqtt_shell(route, inbound)
                case Topic.SPI: await self._handle_mqtt_spi(route, inbound)
                case Topic.DIGITAL | Topic.ANALOG: await self._handle_mqtt_pin(route, inbound)
                case Topic.SYSTEM: await self._handle_mqtt_system(route, inbound)
                case _: pass
    ...
    async def _handle_mcu_status(self, seq_id: int, status: Status, payload: bytes) -> None:
        ...
        await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SYSTEM, Topic.STATUS), msgspec.msgpack.encode({"status": status.value, "name": status.name, "description": status.description, "message": text}), content_type="application/msgpack", user_properties=(("bridge-status", status.name),)))

    # --- MQTT Specific Handlers (Cleaned) ---

    async def _handle_mqtt_console(self, inbound: Message) -> None:
        if pl := msgspec.convert(inbound.payload, bytes): self.state.console_to_mcu_queue.append(pl); await self._flush_console_queue()

    async def _handle_mqtt_datastore(self, route: TopicRoute, inbound: Message) -> None:
        key_parts = list(route.remainder)
        if key_parts and key_parts[-1] in ("request", "response"): key_parts.pop()
        key = "/".join(key_parts); pl = msgspec.convert(inbound.payload, bytes)
        if not key: return
        if route.identifier == DatastoreAction.PUT:
            if len(key.encode()) <= 255 and len(pl) <= 255:
                if self.state.datastore_cache is not None: self.state.datastore_cache[key] = pl
                await self._publish_datastore_value(key, pl, reply_context=inbound)
        elif route.identifier == DatastoreAction.GET:
            cache = cast(Any, self.state.datastore_cache); val = cache.get(key) if cache else None
            if val is not None: await self._publish_datastore_value(key, msgspec.convert(val, bytes), reply_context=inbound)
            elif route.remainder and route.remainder[-1] == "request": await self._publish_datastore_value(key, b"", reply_context=inbound, error="datastore-miss")
    ...
    async def _handle_mqtt_mailbox(self, route: TopicRoute, inbound: Message) -> None:
        ...
        if route.identifier == MailboxAction.WRITE:
            self.state.mailbox_queue.append(pl); await self.serial.send(Command.CMD_MAILBOX_PUSH.value, msgspec.msgpack.encode(MailboxPushPacket(data=pl)))
            ...
            await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.MAILBOX, MailboxAction.READ, protocol.MQTT_SUFFIX_RESPONSE), data), reply_context=inbound)

    async def _handle_mqtt_file(self, route: TopicRoute, inbound: Message) -> None:
        act, target = route.action, "/".join(route.remainder)
        if not (act and target): return
        if target.startswith("mcu/"):
            if act == FileAction.READ: await self._handle_mqtt_file_mcu_read(inbound, target)
            elif act == FileAction.WRITE: await self.serial.send(Command.CMD_FILE_WRITE.value, msgspec.msgpack.encode(FileWritePacket(path=target[4:], data=msgspec.convert(inbound.payload, bytes))))
            elif act == FileAction.REMOVE: await self.serial.send(Command.CMD_FILE_REMOVE.value, msgspec.msgpack.encode(FileRemovePacket(path=target[4:])))
        else:
            ...
            if act == FileAction.WRITE:
                if await self._write_with_quota(path, msgspec.convert(inbound.payload, bytes)):
                    await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.FILE, FileAction.READ, target), msgspec.convert(inbound.payload, bytes)), reply_context=inbound)
            elif act == FileAction.READ and path.is_file():
                if not inbound.topic.value.endswith(protocol.MQTT_SUFFIX_RESPONSE):
                    await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.FILE, FileAction.READ, target, protocol.MQTT_SUFFIX_RESPONSE), await asyncio.to_thread(path.read_bytes)), reply_context=inbound)
            elif act == FileAction.REMOVE and path.exists(): await asyncio.to_thread(path.unlink)
    ...
    async def _handle_mqtt_file_mcu_read(self, ctx: Message, target: str) -> None:
        async with self._mcu_read_lock:
            ...
            if await self.serial.send(Command.CMD_FILE_READ.value, msgspec.msgpack.encode(FileReadPacket(path=target[4:]))):
                try:
                    res = await asyncio.wait_for(self._pending_mcu_read.future, 30.0)
                    await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.FILE, FileAction.READ, protocol.MQTT_SUFFIX_RESPONSE, target), res), reply_context=ctx)
                except asyncio.TimeoutError: pass
            self._pending_mcu_read = None

    async def _handle_mqtt_shell(self, route: TopicRoute, inbound: Message) -> None:
        act = route.segments[0] if route.segments else None; pl = msgspec.convert(inbound.payload, bytes)
        if act == ShellAction.RUN_ASYNC:
            try:
                cmd = msgspec.msgpack.decode(pl, type=ShellCommandPayload).command if pl.startswith(b"\x81") else pl.decode().strip()
                pid = await self._run_process(cmd)
                await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SHELL, ShellAction.RUN_ASYNC, protocol.MQTT_SUFFIX_RESPONSE), str(pid).encode() if pid else b"error:internal"), reply_context=inbound)
            except Exception as exc:
                await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SHELL, ShellAction.RUN_ASYNC, protocol.MQTT_SUFFIX_RESPONSE), f"error:{exc}".encode()), reply_context=inbound)
        elif act in (ShellAction.POLL, ShellAction.KILL) and len(route.segments) == 2:
            pid = int(route.segments[1])
            if act == ShellAction.POLL:
                batch = await self._poll_process(pid); await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SHELL, ShellAction.POLL, str(pid), protocol.MQTT_SUFFIX_RESPONSE), msgspec.msgpack.encode(batch), content_type="application/msgpack"), reply_context=inbound)
            else: await self._stop_process(pid)

    async def _handle_mqtt_spi(self, route: TopicRoute, inbound: Message) -> None:
        match route.identifier:
            case SpiAction.BEGIN: await self.serial.send(Command.CMD_SPI_BEGIN.value, b"")
            case SpiAction.END: await self.serial.send(Command.CMD_SPI_END.value, b"")
            case SpiAction.CONFIG:
                try:
                    p = msgspec.json.decode(inbound.payload, type=SpiConfigPacket); await self.serial.send(Command.CMD_SPI_SET_CONFIG.value, msgspec.msgpack.encode(p))
                except Exception as exc: logger.error("SPI config error: %s", exc)
            case SpiAction.TRANSFER:
                if inbound.payload:
                    res = await self.serial.send_and_wait_payload(Command.CMD_SPI_TRANSFER.value, msgspec.msgpack.encode(SpiTransferPacket(data=bytes(inbound.payload))))
                    if res: await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SPI, SpiAction.TRANSFER, protocol.MQTT_SUFFIX_RESPONSE), msgspec.msgpack.decode(res, type=SpiTransferResponsePacket).data), reply_context=inbound)
    ...
    async def _handle_mqtt_pin(self, route: TopicRoute, inbound: Message) -> None:
        ...
        if len(route.segments) == 2:
            if route.segments[1] == PinAction.MODE: await self.serial.send(Command.CMD_SET_PIN_MODE.value, msgspec.msgpack.encode(PinModePacket(pin=pin, mode=int(pl))))
                ...
                if len(q) < self.state.pending_pin_request_limit: q.append(structures.PendingPinRequest(pin=pin, reply_context=inbound)); await self.serial.send(cmd.value, msgspec.msgpack.encode(PinReadPacket(pin=pin)))
        else:
            cmd = Command.CMD_DIGITAL_WRITE if route.topic == Topic.DIGITAL else Command.CMD_ANALOG_WRITE
            await self.serial.send(cmd.value, msgspec.msgpack.encode(DigitalWritePacket(pin=pin, value=int(pl) if pl.isdigit() else 0)))

    async def _handle_mqtt_system(self, route: TopicRoute, inbound: Message) -> None:
        match route.identifier:
            case SystemAction.BOOTLOADER: await self.serial.send(Command.CMD_ENTER_BOOTLOADER.value, msgspec.msgpack.encode(EnterBootloaderPacket(magic=protocol.BOOTLOADER_MAGIC)))
            case SystemAction.FREE_MEMORY if "get" in route.segments:
                ...
                if pl: await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SYSTEM, SystemAction.FREE_MEMORY, SystemAction.VALUE), str(msgspec.msgpack.decode(pl, type=FreeMemoryResponsePacket).value).encode()), reply_context=inbound)
            case SystemAction.VERSION if "get" in route.segments: await self._request_mcu_version(inbound)
    ...
    async def _request_mcu_version(self, inbound: Message | None = None) -> bool:
        ...
        if pl:
            p = msgspec.msgpack.decode(pl, type=VersionResponsePacket); self.state.mcu_version = (p.major, p.minor, p.patch)
            await self._publish_version(self.state.mcu_version, inbound); return True
        return False

    async def _publish_version(self, v: tuple[int, int, int], ctx: Message | None) -> None:
        pl, tp = f"{v[0]}.{v[1]}.{v[2]}".encode(), topic_path(self.state.mqtt_topic_prefix, Topic.SYSTEM, SystemAction.VERSION, SystemAction.VALUE)
        await self.enqueue_mqtt(QueuedPublish(tp, pl, message_expiry_interval=MQTT_EXPIRY_DATASTORE))
        if ctx: await self.enqueue_mqtt(QueuedPublish(tp, pl, message_expiry_interval=MQTT_EXPIRY_DATASTORE), reply_context=ctx)
    ...
    async def _flush_console_queue(self) -> None:
        while self.state.console_to_mcu_queue and not self.state.mcu_is_paused:
            ...
            for chunk in itertools.batched(buf, protocol.MAX_PAYLOAD_SIZE):
                if not await self.serial.send(Command.CMD_CONSOLE_WRITE.value, msgspec.msgpack.encode(ConsoleWritePacket(data=bytes(chunk)))):
                    self.state.console_to_mcu_queue.appendleft(buf); return

    async def _run_process(self, command: str) -> int:
        if not self.state.allowed_policy.is_allowed(command): return 0
        await self._process_slots.acquire()
        try:
            p = await asyncio.create_subprocess_exec(*shlex.split(command), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, start_new_session=True)
            pid = p.pid & 0xFFFF
            async with self.state.process_lock: self.state.running_processes[pid], self.state.process_io_locks[pid], self.state.process_exit_codes[pid] = p, asyncio.Lock(), 0
            asyncio.create_task(self._monitor_process(pid)); return pid
        except OSError: self._process_slots.release(); return 0

    async def _monitor_process(self, pid: int) -> None:
        try:
            async with self.state.process_lock: h = self.state.running_processes.get(pid)
            if h:
                try: self.state.process_exit_codes[pid] = await asyncio.wait_for(h.wait(), float(self.state.process_timeout))
                except asyncio.TimeoutError:
                    import os, signal
                    with contextlib.suppress(OSError): os.killpg(h.pid, signal.SIGKILL)
                    self.state.process_exit_codes[pid] = -1
        finally: self._finalize_process(pid)

    async def _poll_process(self, pid: int) -> ProcessOutputBatch:
        async with self.state.process_lock:
            h, io_lock, ec = self.state.running_processes.get(pid), self.state.process_io_locks.get(pid), self.state.process_exit_codes.get(pid, 0)
            if not h or not io_lock: return ProcessOutputBatch(Status.ERROR.value, 1, b"", b"", True, False, False)
            async with io_lock:
                async def _rd(s: asyncio.StreamReader | None) -> tuple[bytes, bool]:
                    if not s or s.at_eof(): return b"", False
                    try: return await asyncio.wait_for(s.read(protocol.MAX_PAYLOAD_SIZE - 32), 0.01), not s.at_eof()
                    except asyncio.TimeoutError: return b"", True
                o, to = await _rd(h.stdout); e, te = await _rd(h.stderr); fin = h.returncode is not None
                if fin and (h.stdout is None or h.stdout.at_eof()) and (h.stderr is None or h.stderr.at_eof()): self._finalize_process(pid)
    ...
    async def _stop_process(self, pid: int) -> bool:
        ...
        try:
            import os, signal
            os.killpg(h.pid, signal.SIGTERM); await asyncio.sleep(0.5)
            if h.returncode is None: os.killpg(h.pid, signal.SIGKILL)
        except Exception: pass
        self._finalize_process(pid); return True

    def _finalize_process(self, pid: int) -> None:
        if self.state.running_processes.pop(pid, None): self.state.process_io_locks.pop(pid, None); self._process_slots.release()

    def _get_safe_path(self, p_str: str) -> Path | None:
        root = Path(self.config.file_system_root).resolve(); p = root.joinpath(p_str.lstrip("/")).resolve()
        return p if p.is_relative_to(root) else None

    async def _write_with_quota(self, path: Path, data: bytes) -> bool:
        async with self._storage_lock: path.parent.mkdir(parents=True, exist_ok=True); await asyncio.to_thread(path.write_bytes, data); return True

    def _parse_pin(self, s: str) -> int:
        s = s.upper(); return int(s[1:]) if s.startswith("A") and s[1:].isdigit() else (int(s) if s.isdigit() else -1)

    def _deduce_action(self, r: TopicRoute) -> str | None:
        if r.topic == Topic.SYSTEM: return None
        if r.topic in (Topic.DIGITAL, Topic.ANALOG): return "write" if len(r.segments) == 1 else (r.segments[1].lower() if len(r.segments) > 1 else None)
    ...
    async def _reject_mqtt(self, ctx: Message, tp: Topic | str, act: str) -> None:
        ...
        await self.enqueue_mqtt(QueuedPublish(topic_path(self.state.mqtt_topic_prefix, Topic.SYSTEM, Topic.STATUS), msgspec.msgpack.encode({"status": "forbidden", "topic": val, "action": act}), content_type="application/msgpack", user_properties=(("bridge-error", TOPIC_FORBIDDEN_REASON),)), reply_context=ctx)

    async def _publish_datastore_value(self, key: str, val: bytes, reply_context: Message | None = None, error: str | None = None) -> None:
        tp = topic_path(self.state.mqtt_topic_prefix, Topic.DATASTORE, DatastoreAction.GET, *filter(None, key.split("/")))
        props = (("bridge-datastore-key", key), ("bridge-error", error)) if error else (("bridge-datastore-key", key),)
        await self.enqueue_mqtt(QueuedPublish(tp, val, message_expiry_interval=MQTT_EXPIRY_DATASTORE, user_properties=props), reply_context=reply_context)
