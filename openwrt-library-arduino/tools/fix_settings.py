...
if start != -1:
    new_code = [
        ...
        '        fs_root = raw_config["file_system_root"]\n',
        '        raw_config["file_system_root"] = os.path.abspath(os.path.expanduser(fs_root))\n',
        '    if "mqtt_spool_dir" in raw_config:\n',
        '        spool_dir = raw_config["mqtt_spool_dir"]\n',
        '        raw_config["mqtt_spool_dir"] = os.path.abspath(os.path.expanduser(spool_dir))\n',
        '    if "mqtt_topic" in raw_config:\n',
        '        if isinstance(raw_config["mqtt_topic"], str):\n',
        '            raw_topic = raw_config["mqtt_topic"]\n',
        '            segments = (s.strip() for s in raw_topic.split("/") if s.strip())\n',
        ...
        '             msg = "mailbox_queue_bytes_limit must be >= mailbox_queue_limit"\n',
