...
El contenido del payload es específico para cada `command_id`. Su longitud máxima es de **256 bytes**.
...
1.  La trama completa (`Header` + `Payload` + `CRC`) se codifica utilizando el algoritmo **COBS**.
...
### 3.1. Códigos de Estado (MCU -> Linux)

| ID (Hex) | Nombre | Descripción |
| :--- | :--- | :--- |
| `0x00` | `STATUS_OK` | Indica que el comando fue recibido y ejecutado con éxito. |
| `0x01` | `STATUS_ERROR` | Error genérico durante la ejecución del comando. |
| `0x02` | `STATUS_CMD_UNKNOWN` | El `command_id` recibido no es reconocido. |
| `0x03` | `STATUS_MALFORMED` | El paquete recibido tiene un formato incorrecto (definido para uso futuro). |
| `0x04` | `STATUS_CRC_MISMATCH` | El CRC del paquete no coincide (definido para uso futuro). |
| `0x05` | `STATUS_TIMEOUT` | Ocurrió un timeout esperando una respuesta (definido para uso futuro). |
| `0x06` | `STATUS_NOT_IMPLEMENTED` | El comando es válido pero no está implementado (definido para uso futuro). |
| `0x07` | `STATUS_ACK` | Acuse de recibo genérico para comandos que no tienen una respuesta específica. |

### 3.2. Comandos de Sistema y Flujo
...
### 3.3. Comandos de I/O de Pines

| ID (Hex) | Nombre | Dirección | Payload | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| `0x10` | `CMD_SET_PIN_MODE` | Linux -> MCU | `[pin (1B)][mode (1B)]` | Configura el modo de un pin (0=IN, 1=OUT, 2=PULLUP). |
...
| `0x15` | `CMD_DIGITAL_READ_RESP`| MCU -> Linux | `[pin (1B)][value (2B)]` | Respuesta a `CMD_DIGITAL_READ`. |
| `0x16` | `CMD_ANALOG_READ_RESP` | MCU -> Linux | `[pin (1B)][value (2B)]` | Respuesta a `CMD_ANALOG_READ`. |

### 3.4. Comandos de Consola

| ID (Hex) | Nombre | Dirección | Payload | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| `0x20` | `CMD_CONSOLE_WRITE` | Bidireccional | `[string (N Bytes)]` | Envía una cadena de texto a la consola del otro procesador. |

### 3.5. Comandos de DataStore (Clave-Valor)

| ID (Hex) | Nombre | Dirección | Payload | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| `0x30` | `CMD_DATASTORE_PUT` | Linux -> MCU | `[key]\0[value]` | Guarda un par clave-valor. |
| `0x31` | `CMD_DATASTORE_GET` | Linux -> MCU | `[key]` | Solicita el valor asociado a una clave. |
| `0x81` | `CMD_DATASTORE_GET_RESP` | MCU -> Linux | `[key]\0[value]` | Respuesta a `CMD_DATASTORE_GET`. |

### 3.6. Comandos de Mailbox (Mensajería)

| ID (Hex) | Nombre | Dirección | Payload | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| `0x40` | `CMD_MAILBOX_READ` | Linux -> MCU | Vacío | Solicita leer el siguiente mensaje de la cola de Linux. |
| `0x41` | `CMD_MAILBOX_PROCESSED`| MCU -> Linux | `[mensaje (N Bytes)]` | Envía un mensaje desde el MCU a Linux. |
| `0x42` | `CMD_MAILBOX_AVAILABLE`| Linux -> MCU | Vacío | Pregunta cuántos mensajes hay disponibles en la cola de Linux. |
| `0x90` | `CMD_MAILBOX_READ_RESP`| MCU -> Linux | `[mensaje (N Bytes)]` | Respuesta a `CMD_MAILBOX_READ`. |
| `0x92` | `CMD_MAILBOX_AVAILABLE_RESP`| MCU -> Linux | `[count (String)]` | Respuesta a `CMD_MAILBOX_AVAILABLE`. |

### 3.7. Comandos de File I/O

| ID (Hex) | Nombre | Dirección | Payload | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| `0x50` | `CMD_FILE_WRITE` | Linux -> MCU | `[path]\0[data]` | Escribe datos en un fichero en el sistema de Linux. |
| `0x51` | `CMD_FILE_READ` | Linux -> MCU | `[path]` | Solicita leer el contenido de un fichero. |
| `0x52` | `CMD_FILE_REMOVE` | Linux -> MCU | `[path]` | Solicita eliminar un fichero. |
| `0xA1` | `CMD_FILE_READ_RESP` | MCU -> Linux | `[data]` | Respuesta a `CMD_FILE_READ` con el contenido del fichero. |

### 3.8. Comandos de Process (Ejecución de Comandos)

| ID (Hex) | Nombre | Dirección | Payload | Descripción |
| :--- | :--- | :--- | :--- | :--- |
| `0x60` | `CMD_PROCESS_RUN` | Linux -> MCU | `[command]` | Ejecuta un comando en Linux y espera a que termine. |
| `0x61` | `CMD_PROCESS_RUN_ASYNC` | Linux -> MCU | `[command]` | Ejecuta un comando en segundo plano. |
| `0x62` | `CMD_PROCESS_POLL` | Linux -> MCU | `[pid]` | Obtiene la salida de un comando asíncrono. |
| `0x63` | `CMD_PROCESS_KILL` | Linux -> MCU | `[pid]` | Termina un proceso en ejecución. |
| `0xB0` | `CMD_PROCESS_RUN_RESP` | MCU -> Linux | `[stdout]` | Respuesta a `CMD_PROCESS_RUN`. |
| `0xB1` | `CMD_PROCESS_RUN_ASYNC_RESP` | MCU -> Linux | `[pid]` | Respuesta a `CMD_PROCESS_RUN_ASYNC`. |
| `0xB2` | `CMD_PROCESS_POLL_RESP` | MCU -> Linux | `[stdout]` | Respuesta a `CMD_PROCESS_POLL`. |