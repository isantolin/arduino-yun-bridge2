...
void test_rle_gaps() {
  ...
  uint8_t dst[2];
  TEST_ASSERT(rle::encode(etl::span<const uint8_t>(src, 2),
                          etl::span<uint8_t>(dst, 2)) == 0);

  // Line 99-105: runs of 0xFF < MIN_RUN_LENGTH
  uint8_t src2[] = {0xFF, 0xFF, 0xFF};
  uint8_t dst2[10];
  size_t len = rle::encode(etl::span<const uint8_t>(src2, 3),
                           etl::span<uint8_t>(dst2, 10));
  TEST_ASSERT(len > 0);

  // Line 147: Decode special 255
  uint8_t decoded[10];
  size_t dlen = rle::decode(etl::span<const uint8_t>(dst2, len),
                            etl::span<uint8_t>(decoded, 10));
  TEST_ASSERT(dlen == 3);
  ...
  uint8_t src4[] = {0xFF};
  uint8_t dst4[10];
  size_t len4 = rle::encode(etl::span<const uint8_t>(src4, 1),
                            etl::span<uint8_t>(dst4, 10));
  TEST_ASSERT(len4 == 3);
  TEST_ASSERT(dst4[1] == 255);

  // Line 147: Decode single 0xFF
  uint8_t decoded4[10];
  size_t dlen4 = rle::decode(etl::span<const uint8_t>(dst4, len4),
                             etl::span<uint8_t>(decoded4, 10));
  TEST_ASSERT(dlen4 == 1);
...
static void timeout_trampoline(rpc::StatusCode status, const uint8_t*,
                               uint16_t) {
...
void test_fsm_gaps_more() {
  ...
  StateUnsynchronized s_un;
  TEST_ASSERT(s_un.on_event_unknown(EvReset()) ==
              etl::ifsm_state::No_State_Change);
  ...
  TEST_ASSERT(s_idle.on_event_unknown(EvReset()) ==
              etl::ifsm_state::No_State_Change);

  StateAwaitingAck s_ack;
  TEST_ASSERT(s_ack.on_event_unknown(EvReset()) ==
              etl::ifsm_state::No_State_Change);

  StateFault s_fault;
  TEST_ASSERT(s_fault.on_event_unknown(EvReset()) ==
              etl::ifsm_state::No_State_Change);
...
void test_services_gaps() {
  ...
  TEST_ASSERT(dsa.trackPendingKey(
                  "this_key_is_way_too_long_for_the_buffer_limit") == false);
  ...
  // Minimum 6 bytes for ProcessPollResponse: status(1), exit(1), out_len(2),
  // err_len(2)
