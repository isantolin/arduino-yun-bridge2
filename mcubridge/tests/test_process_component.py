...
async def test_finalize_process(process_comp: ProcessComponent) -> None:
    ...
    assert pid in process_comp.state.running_processes
    # 2 - 1 = 1
    assert process_comp.process_slots._value == 1  # type: ignore[reportPrivateUsage]

    await process_comp.finalize_process(pid)  # type: ignore[reportPrivateUsage]

    assert pid not in process_comp.state.running_processes
    assert (
        process_comp.process_slots._value  # type: ignore[reportPrivateUsage, reportUnknownMemberType]
        == 2
