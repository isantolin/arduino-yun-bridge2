...
async def test_client_digital_write(mock_client) -> None:
    ...
    assert "br/cmd/d/13" in last_call.args[0]
...
async def test_client_datastore_put(mock_client) -> None:
    ...
    async def simulate_response(*args, **kwargs):
        # The response topic for cmd/datastore/put/key is stat/datastore/get/key
        topic = args[0]
        if "cmd/datastore/put/" in topic:
            key = topic.split("/")[-1]
            resp_topic = f"br/stat/datastore/get/{key}"
    ...
    put_call = next(c for c in client_instance.publish.call_args_list if "br/cmd/datastore/put/test_key" in c.args[0])
...
async def test_client_file_write(mock_client) -> None:
    ...
    assert client_instance.publish.called
    last_call = client_instance.publish.call_args_list[-1]
    assert "br/cmd/file/write/test.txt" in last_call.args[0]
