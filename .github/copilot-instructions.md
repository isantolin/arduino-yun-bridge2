...
4.  **Universal Absolute Suppression Ban & Strict Encoding Integrity:** Suppressions of any kind are strictly prohibited across the entire repository (production code, firmware C++, tools, helper scripts, and test suites); specifically, all warnings MUST be treated as errors, and "Pokemon" exceptions (catch-all) as well as silencing mechanisms such as errors="ignore", errors="replace", or errors="backslashreplace" on decode/encode, contextlib.suppress, except: pass, or # type: ignore / # pyright: ignore comments are strictly forbidden, with the sole exception of autogenerated code (stubs *_pb2.py). If any suppressions are found in the codebase, they must be removed immediately. All string/byte transformations must be strict UTF-8 to ensure deterministic failure on malformed input. Mocking in tests must utilize typed local variables (mock_fn = AsyncMock(); setattr(sut, "fn", mock_fn); mock_fn.assert_awaited()) ensuring complete, unsuppressed static type validation.
...
9.  **Eradication of Passthrough Methods & Declarative Routing:** Prohibit passthrough/proxy wrapper methods whose sole purpose is delegating a call to an internal member without business logic. Access the member directly. Routing and dispatch tables (_topic_dispatch, _shell_dispatch, _spi_dispatch, local_dispatch) MUST bind directly to canonical handler methods or compact direct callables (e.g. lambda pid, _: self.kill_process(pid)), eliminating 1-line intermediate wrapper functions.
