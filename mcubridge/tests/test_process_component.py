"""Tests for the ProcessComponent using native asyncio mocks."""
...
async def test_finalize_process(process_component: ProcessComponent) -> None:
    ...
    await process_component._finalize_process(pid, 0)

    assert pid not in process_component.state.running_processes
    assert process_component._process_slots._value == 2
